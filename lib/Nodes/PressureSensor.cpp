...
void PressureSensor::sendMeasurement(float value, ChangeValueReason reason) const {

    float pressureHpa = value / 100.0f;

    Homie.getLogger() << mNode.cIndent << getName() << F(": ") << pressureHpa << " " << cUnitHpa << " due reason: " << (int)reason << endl;
